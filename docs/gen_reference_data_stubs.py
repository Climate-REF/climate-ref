...
from the same function `ref doctor` uses, so the published table and the local check cannot disagree.
...
# Providers whose requirements make up the reference data list.
# The example provider is excluded
PROVIDERS = [
