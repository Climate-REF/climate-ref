...
(NumPy, SciPy, and the BLAS/LAPACK backends they link against, such as OpenBLAS
or MKL) spin up their own thread pools. By default each of these pools grows to
the number of CPU cores on the machine. Because the REF runs many diagnostic
executions in parallel (the [`LocalExecutor`](executors.md) launches up to `n`
worker processes), an unconfigured BLAS backend multiplies out to
`n_processes * n_cores` threads, which oversubscribes the CPUs and causes heavy
context-switching that slows the whole run down.

Cap the per-process thread count by exporting the following environment
variables before running `ref solve` (a small number such as `4` is a good
starting point; tune it against the executor's `n` and your core count):
...
Each variable targets a different backend, so it is safest to set all of them:
`OMP_NUM_THREADS` (OpenMP), `OPENBLAS_NUM_THREADS` (OpenBLAS),
`MKL_NUM_THREADS` (Intel MKL), `NUMEXPR_NUM_THREADS` (NumExpr),
`VECLIB_MAXIMUM_THREADS` (Apple Accelerate/vecLib), and `BLIS_NUM_THREADS`
(BLIS). They must be set in the environment *before* NumPy is imported, so
export them in the shell (or your job script) that launches the solve rather
than from within Python.

/// note
As a rough guide, keep `n * OMP_NUM_THREADS` at or below the number of physical
cores so the executor's worker processes and the BLAS threads do not compete for
the same CPUs.
///
