...
def test_parse_rejects_unknown_mip_table():
    # An unrecognised MIP table cannot be reduced to a frequency and must be flagged,
    # otherwise a mis-parsed path could collapse distinct datasets onto one slug.
    result = parse_esmvaltool_reference(
        "/root/ESMValTool/OBS/Tier2/FOO/OBS_FOO_sat_v1_NotATable_tas_200001-200012.nc"
    )
    assert "INVALID_ASSET" in result
...
def reference_tree(tmp_path) -> Path:
    ...
    files = [
        ...
        "OBS/Tier2/OSI-450-nh/OBS_OSI-450-nh_reanaly_v3_SImon_siconc_197901-197912.nc",
        "OBS/Tier2/OSI-450-nh/OBS_OSI-450-nh_reanaly_v3_SImon_siconc_198001-198012.nc",
...
def test_ingest_end_to_end(reference_tree, db):
    ...
    assert set(catalog["instance_id"]) == {
        "esmvaltool-reference.OBS.CERES-EBAF.mon.rlut.Ed4.2",
        "esmvaltool-reference.OBS.OSI-450-nh.mon.siconc.v3",
    ...
    assert counts["esmvaltool-reference.OBS.OSI-450-nh.mon.siconc.v3"] == 2
